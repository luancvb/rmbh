...
- **Mensagem atual.** Chama para o panfletaço do Renan no domingo, 20/09, às 11h, na Feira
  Hippie (Av. Afonso Pena, 705, Centro), e pede confirmação de presença pelo formulário
  <https://forms.gle/rp3hgNbaAYwFcPAz5>. Passado o dia 20, troque o texto em `MSG_PADRAO`
  (`assets/js/app.js`).
- **Contagem regressiva automática.** O trecho `{contagem}` vira a frase pronta — "Faltam só
  16 dias para a eleição.", "Falta só 1 dia para a eleição.", "A eleição é hoje!" — calculada
  no instante do clique até **03/10**. `{dias}` traz só o número, se você quiser escrever a
  frase do seu jeito. Para mirar outra data, mude a linha `DATA_ALVO` no começo do
  `assets/js/app.js`.
